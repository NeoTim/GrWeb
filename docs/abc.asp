...
if oMatches.Count>0 then  
	if bmtype=1 then
		bentitle = iconv("GB2312","UTF-8",oMatches(0). SubMatches(0)) 'UTF-8转GB2312
	else
...
'				end if
		...
		for xxi=0 to adsdfsdafwe
			if arrxxx(xxi,5)=1 or t=3 then
				...
				if lei = 1 then
					...
					echo nowdir&"/"&arrxxx(xxi,4)&"/"&(xxi+1)&".html 生成ok<br>"
				else
					html tconnei,nowdir&"/"&arrxxx(xxi,4)&"/index.html"
					echo nowdir&"/"&arrxxx(xxi,4)&"/index.html 生成ok<br>"
				end if
				Response.Flush
				Response.Clear
			end if '内页
			...
			if bbbxx(lllll)<>"" then
				...
				for lmsxi=0 to lmcdss
					...
					if lmsxi=pagesize*(kk+1)-1 or lmsxi = ubound(lmsx)-1 then
						...
						next
						pagede = "<li><span class=""pageinfo"">当前页数 <strong>"&kk&"</strong> 共 <strong>"&ys&"</strong> 页<strong> "&lmcdss+1&" 条记录</strong></span></li>"
		...
		' 生成首页	
			...
			shoucon = replace(shoucon,"{link}",youlink) '友情链接
			
			html shoucon,nowdir&"/index.html"
			echo nowdir&"/index.html 生成ok<br>"
			echo "<br>"
	end if
next
...
Function AutoCreateFolder(strPath) ' As Boolean
		...
		strPath =server.MapPath(".")&"\"&strPath  '当前目录下的..
